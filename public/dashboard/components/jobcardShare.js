...
function openShareDialog({item,url,type}){
    ...
    const jobcardEmailFlow=isTask?"":`<section class="dashboard-share-email-flow" data-email-flow><button type="button" class="dashboard-button dashboard-button-secondary" data-email>${escapeHtml(t("email","Email"))}</button><section class="dashboard-share-deadline" data-deadline hidden><label>${escapeHtml(t("shareDeadline","Completion deadline"))}<input class="dashboard-input" type="date" data-deadline-input></label><button type="button" class="dashboard-button" data-deadline-confirm disabled>${escapeHtml(t("confirmDeadline","Confirm date"))}</button></section></section>`;
