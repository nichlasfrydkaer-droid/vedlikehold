...
import { getJobcards,getJobcardSettings,getTasks,getReports,getActivity,getPlanner } from "../js/api.js";
import { getCongregation } from "../js/session.js";
import { formatExecutionMonth, mergeJobcardSchedules, isUpcoming } from "../js/jobcardSchedule.js";
import { getTaskStatus,renderTaskStatus } from "../js/taskStatus.js";
import { t } from "../js/i18n.js";
import { reportCompletionIndex,scheduledForYear } from "./planner.js";
...
export async function initDashboard(){
  ...
  const reportCompletions=reportCompletionIndex(reports);
  const plannerYears=new Map();
  ...
  const plannerKey=value=>{const key=String(value??"").trim();const numeric=key.match(/^0*(\d+)([A-Za-z]?)$/);return numeric?`${Number(numeric[1])}${numeric[2].toUpperCase()}`:key.toUpperCase();};
  const loadPlannerYear=async year=>{
    if(plannerYears.has(year))return plannerYears.get(year);
    const result=await getPlanner(congregation.id,year);
    const manualCompletions=new Set();
    (result?.manualCompletions||[]).forEach(item=>manualCompletions.add(`${plannerKey(item.jobcard_id)}:${item.execution_month}`));
    const data={entries:result?.success?result.entries||[]:[],manualCompletions};
    plannerYears.set(year,data);
    return data;
  };
  const isOverdueExecution=executionMonth=>executionMonth<`${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,"0")}`;
  const jobcardMonthSection=(items,limit)=>{
    const completed=items.filter(item=>item.completed).length;
    return `<div class="dashboard-month-section dashboard-planner-month"><div class="dashboard-month-section-heading"><h3>${t("jobcards","Jobbkort")}</h3>${items.length?`<span>${completed}/${items.length} ${t("completed","Utført").toLowerCase()}</span>`:""}</div>${items.slice(0,limit).map(item=>{const overdue=!item.completed&&isOverdueExecution(item.executionMonth);return `<a href="/dashboard/annual-overview/?year=${encodeURIComponent(shownMonth.getFullYear())}" class="dashboard-planner-jobcard ${item.completed?"is-completed":overdue?"is-overdue":"is-pending"}"><span class="dashboard-planner-status" aria-label="${item.completed?t("completed","Utført"):overdue?t("overdueJobcards","Overskredne jobbkort"):t("planned","Planlagt")}">${item.completed?"✓":overdue?"!":""}</span><span><strong>${esc(item.jobcard_number)}</strong>${esc(item.title)}</span><small>${item.completed?t("completed","Utført"):overdue?t("overdueJobcards","Overskredne jobbkort"):t("planned","Planlagt")}</small></a>`;}).join("")||`<p>${t("noJobcardsPlanned","Ingen planlagte jobbkort.")}</p>`}</div>`;
  };
  ...
  const render=async()=>{
    ...
    const plan=await loadPlannerYear(shownMonth.getFullYear());
    const monthlyCards=scheduledForYear(jobcards,shownMonth.getFullYear(),reportCompletions,plan.entries,plan.manualCompletions)[shownMonth.getMonth()];
    const currentPlan=shownMonth.getFullYear()===today.getFullYear()?plan:await loadPlannerYear(today.getFullYear());
    const currentYearSchedule=scheduledForYear(jobcards,today.getFullYear(),reportCompletions,currentPlan.entries,currentPlan.manualCompletions);
    const overdueCards=currentYearSchedule.slice(0,today.getMonth()).flat().filter(item=>!item.completed);
    const limit=expanded?999:3;
    const overdue=tasks.filter(item=>getTaskStatus(item)==="overdue").length;
    root.querySelector("#dashboardContent").innerHTML=`<main class="dashboard-home" id="dashboardContent"><section class="dashboard-stat-grid"><a href="/dashboard/tasks.html" class="dashboard-stat danger"><strong>${overdue}</strong><span>Overskredne oppgaver</span></a><a href="/dashboard/tasks.html" class="dashboard-stat"><strong>${tasks.filter(item=>getTaskStatus(item)==="open").length}</strong><span>Åpne oppgaver</span></a><a href="/dashboard/annual-overview/?year=${today.getFullYear()}" class="dashboard-stat danger"><strong>${overdueCards.length}</strong><span>Overskredne jobbkort</span></a><a href="/dashboard/reports.html" class="dashboard-stat"><strong>${reports.filter(item=>String(item.finished_at||item.created_at||"").startsWith(currentMonthKey)).length}</strong><span>Rapporter denne måned</span></a></section><section class="dashboard-home-card dashboard-month-card"><header><button data-month="-1" aria-label="Forrige måned">‹</button><h2>Oppgaver for ${monthName(shownMonth)}</h2><button data-month="1" aria-label="Neste måned">›</button></header>${monthSection("Oppgaver",monthlyTasks,"task",limit)}${jobcardMonthSection(monthlyCards,limit)}${monthlyTasks.length>3||monthlyCards.length>3?`<button class="dashboard-expand" data-expand>${expanded?"Vis færre":"Vis alle"}</button>`:""}</section><div class="dashboard-two-column">${listCard("Kommende oppgaver",upcomingTasks,"task","/dashboard/tasks.html")}${listCard("Kommende jobbkort",upcomingCards,"jobcard","/dashboard/jobcards.html")}</div><section class="dashboard-home-card dashboard-activity-card"><header><h2>Aktivitet</h2><a href="#" aria-disabled="true">Se all aktivitet</a></header><div class="dashboard-activity-list">${activityRows()}</div></section></main>`;
    const labels=[t("overdueTasks","Overskredne oppgaver"),t("openTasks","Åpne oppgaver"),t("overdueJobcards","Overskredne jobbkort"),t("reportsThisMonth","Rapporter denne måned")];
    ...
    root.querySelector(".dashboard-month-section h3").textContent=t("tasks","Oppgaver");
    ...
    const emptyTask=root.querySelector(".dashboard-month-section:not(.dashboard-planner-month) p");
    if(emptyTask)emptyTask.textContent=t("noPlanned","Ingen planlagte {items}.").replace("{items}",t("tasks","oppgaver"));
